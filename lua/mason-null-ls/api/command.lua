...
local function parse_packages_from_user_args(user_args)
	...
	local language_mappings = require('mason-null-ls.mappings.language')
	local language_map = language_mappings.get_language_map()
	...
	return _.filter_map(function(source_specifier)
		...
		return Optional
			...
			:or_(function()
				return Optional.of_nilable(language_map[source_name]):map(function(package_names)
...
_G.mason_null_ls_completion = {
	available_source_completion = function()
		...
		local completions = sort_deduped(_.concat(_.keys(language_mappings.get_language_map()), available_sources))
